...
| **Parallel with** | Task 4 (Component Storage) — separate worktree |
| **Skill** | `subagent-driven-development` or `executing-plans` |

## Shared File Notes

This task adds `pub mod item;` to `lib.rs`. If running in parallel with Task 4, both branches will modify `lib.rs`. This is a trivial additive conflict — resolve at merge time by keeping both `pub mod` lines. Use `claim_file("crates/factorial-core/src/lib.rs")` when merging.

## Files

- Create: `crates/factorial-core/src/item.rs`
- Modify: `crates/factorial-core/src/lib.rs` — add `pub mod item;`

## Context

Design doc §2 "Items" and §9 "Item Instance Storage". Fungible items = counter. Stateful items = individually tracked in typed arenas per item type.

## Step 1: Write item storage types and tests

`crates/factorial-core/src/item.rs`:

```rust
use crate::fixed::{Fixed64, Fixed32};
use crate::id::*;
...
```bash
cargo test -p factorial-core && git add -A && git commit -m "feat: item storage with fungible inventory slots"
```

## Verification

- `cargo test -p factorial-core -- item::tests` — all tests pass
- `InventorySlot::add` returns correct overflow
- `InventorySlot::remove` returns correct removed count

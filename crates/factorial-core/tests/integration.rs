...
fn oxygen() -> ItemTypeId {
    ItemTypeId(4)
}
fn hydrogen() -> ItemTypeId {
    ItemTypeId(5)
}
...
/// Helper: add a node to the engine, set processor and inventories.
...
/// Helper: connect two nodes and set transport. Returns the EdgeId.
...
/// Helper: get the total quantity of a specific item in a node's output inventory.
fn output_quantity(engine: &Engine, node: NodeId, item: ItemTypeId) -> u32 {
    engine
...
/// Helper: get the total quantity of a specific item in a node's input inventory.
fn input_quantity(engine: &Engine, node: NodeId, item: ItemTypeId) -> u32 {
    engine
...
/// Helper: total items across all types in a node's input inventory.
fn input_total(engine: &Engine, node: NodeId) -> u32 {
    engine
        .get_input_inventory(node)
...
/// Helper: total items across all types in a node's output inventory.
fn output_total(engine: &Engine, node: NodeId) -> u32 {
    engine
        .get_output_inventory(node)
